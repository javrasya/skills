...
- **What `worker-start` cannot carry stays out of reach.** There is no permission-mode flag, and `--model`/`--effort` reach Claude, Codex and Cursor only — so a pi worker's model, effort and trust are not passed through this path. The tickets that own agent options deal with it; the fallback path remains the way to reach them if one needs to.
