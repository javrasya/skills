...
**A worker is watchable only if its start receipt says so.** The terminal comes from the operator's new-agent-tab setting, not from anything the runner asks for. The runner reads `mode.mode` and the worker's terminal handle from the receipt, and reports a worker without a terminal loudly, naming the setting, rather than running it unwatched.
...
- **Watchability depends on one operator setting.** An operator whose new agent tabs are not terminals gets told so at the first worker, not an invisible run.
- **What `worker-start` cannot carry comes in on the agent's own command line, and the worker is still supervised.** There is no permission-mode flag, and `--model`/`--effort` reach Claude, Codex and Cursor only — so a pi worker's model, effort and trust are not passed through `--agent`. Such a worker (a Claude worker given the orchestrator's permission mode, and every pi worker, started with `--approve`) is launched by Orca's documented route for custom argv: the runner creates the agent's terminal with that command, waits for its TUI to go idle, and hands the terminal to `worker-start --terminal`, which supervises it like any other worker (#28). Orca's release retains a terminal the worker did not create, so the runner closes it after release.
