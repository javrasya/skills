...
- **Leftover worktrees stop mattering for correctness.** They still cost disk, but no agent needs a branch name freed in order to work. *(Amended 2026-09-03: they are no longer left until finalize — the publish lane reclaims a ticket's worktrees the moment its PR exists, and finalize takes only what the lane could not. See "Worktree reclaim" in `CONTEXT.md`.)*
