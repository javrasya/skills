...
| `earlier` | a resume carries forward, before any call, every other agent of the Run an earlier runner made: one that settled, or whose worker never started but left a worktree. It is no call, and replays nothing; it keeps the agent named for reclaim and the run view | `n`, `title`, `run`, `dispatchId`, `harness`, `sessionId`, `terminal`, `worktree`, `origin`, `state` and `reason` (as the journal last had them), `continuations` if its session was continued, a doctor's `patient`, which keeps it under its patient in the run view, and a patient's `round` and `rounds`, as the fold had them |
...
| `gaveUp` | a doctor round ended without a remedy; after the last, the patient's `failed` follows | `key`, `n`, `title` and `origin` (the patient's), `round`, `doctor`, `reason` (why: it gave up over mail, with its `worker_done --outcome failed` body, it settled with no remedy, or it failed itself) |
| `mail` | the runner read a message from its Run mailbox, which it reads from its own terminal while a doctor is out. Journaled before the runner acts on it and acknowledges it: Orca delivers a batch again until it is acknowledged, and a message id already journaled is acknowledged and never acted on again. A resume carries each one forward before any call | `messageId`, `kind` (Orca's message type: `handoff`, `worker_done`…), `action` (`remedy`: a doctor's note carried its patient on; `gaveUp`: its doctor gave up; `ended`: its doctor's `worker_done --outcome succeeded`; `none`: from no doctor out, or asking for nothing), `outcome` for a `worker_done`, and for a doctor's message its `doctor` (`n`), `patient` (origin), `round` and `body` (the note, or why it gave up). No `n`: it is about no agent's lifecycle |
| `remedy` | a doctor's handoff carried its patient on: the patient's session continued, in its own session and worktree, in its own tab or a new one in its worktree once that tab was gone, with the note in its continuation prompt. It counts against no continuation cap, and starts the patient's count afresh: its next `continued` is attempt 1 | `key`, `n`, `title` and `origin` (the patient's), `round`, `doctor`, `how` (`continue`), `messageId` (the handoff's), `dispatchId` and `terminal` (the ones it now runs under), `reopened` (true when its tab was gone) |
...
A doctor's `handoff` is its remedy: the runner, which reads its Run mailbox while a doctor is out, continues the patient's session with the note in its continuation prompt, and the patient's result is what its `agent()` returns. A doctor that ends without a remedy spends its round: it gives up (`worker_done --outcome failed`), its worker settles with no handoff, or it fails itself after its own start retries and continuations. A doctor is never doctored. After the third round without a remedy the patient's call returns null: it is failed and **kept**, as before, its process not stopped, its tab open and its worktree retained.

Rounds build on each other. A remedy starts the patient's continuations afresh: its next death is continuation 1 of 3 again, and only a death past that cap starts the next round. Round *k*'s doctor prompt has an `Earlier doctor rounds` section with each earlier round's note (none for a doctor that gave up) and how that round ended, so it never repeats a note that already failed. The journal fold gives each patient its `round`, its `rounds` (`{ round, doctor, reason, outcome, note, why }`, `outcome` being `remedy`, `gaveUp`, or null while it runs) and its `doctors`, and each doctor its `patient`; a resume carries the patient's `round` and `rounds` on its `earlier` line.
...
| a dead agent gets its doctors (ADR-0014) | `orca-contract:doctored` runs a nine-minute wait, and you kill it every time it runs: once, then in each of its 3 continuations, four kills in all. Past its cap it is the patient of three doctor rounds, one after another: each a `[Contract] recover -> orca-contract:doctored` worker in its own `<runId>-<n>` worktree, created with setup skipped. Its prompt tells a doctor to give up, so you kill no doctor: each sends `worker_done --outcome failed`, journaled as `mail` with `action: gaveUp`, and the log says `doctor round <k> of 3 ended without a remedy: it gave up: …`. The run view shows each doctor as a `└ recover` row under its patient | `doctored: null`, only after the third round: the log's `agent() returns null, after 3 doctor rounds without a remedy` comes before `orca-contract:doctored returned` |
| a doctor's note carries its patient on | `contract:doctor`'s prompt says to finish only once a later message gives it the word `PROCEED`, and until then to stop without submitting, however often it is told to finish. So it goes idle without submitting in its session and in each of its 3 continuations, twice nudged each time, and past its cap it is the patient of `[Contract] recover -> contract:doctor`. Nothing to do by hand: its prompt asks a doctor to read its transcript and hand off a note that gives it the word. The runner journals the doctor's `handoff` as `mail` (`action: remedy`) and then a `remedy` line, and logs `doctor round 1 handed off a note; continuing session … with it`; the patient submits, and the doctor's `worker_done --outcome succeeded` is journaled as `mail` with `action: ended` | `doctor: {count: 3, word: "hello"}`, after the `remedy` line and with no `gaveUp` for it |

Every run, fresh or resumed, must return exactly this:

```json
{"returned":{"count":3,"word":"hello"},"dirtyRetry":{"count":3,"word":"hello"},"doctor":{"count":3,"word":"hello"},"doctored":null,"failures":[]}
```
...
1. **Launch** and keep `result.terminal.handle` as `<runner>`:
   ...
   ```
   `--require` preloads the faults the cases need (`scripts/runner-contract-orca-fault.cjs`). A fresh run takes over two hours: `contract:dirty-retry`'s held create and `contract:doctor`'s idle sessions and doctor run side by side for about half an hour, and `orca-contract:doctored`'s kills and three doctors take the rest.
...
6. **Clean up:** close the kept worker tabs, named on the `!! kept …, tab <handle>` log lines, and the runner tabs with `orca terminal close`. Remove any `<runId>-<n>` child worktree, `contract:dirty-retry`'s included, with `orca worktree rm` (add `--force`: it holds the untracked file). Each run leaves one per doctor too.
...
Last pass: 2026-09-26 (#74), Orca 1.4.212 and Claude Code on macOS, with the runner as of #74's doctor rounds (89f3bd6: a fresh continuation count per round, earlier rounds in the doctor prompt, `rounds` in the fold), driven by an agent with the steps above, `orca terminal` verbs and the state dir's files only, launched with the preload. **The Orca-only contract holds, fresh and resumed** (Run `run_d402b0242bb5`). No case of the contract hands a patient a second round after a remedy: that is covered offline only.

- **Fresh:** `orca-contract:returned` was nudged once and submitted. `contract:dirty-retry`'s held create timed out at 600 s and it started in `run_d402b0242bb5-2`, which Orca had made. `contract:doctor` went idle in its session and its 3 continuations, and past its cap its doctor (`-4`) handed off a `PROCEED` note: `mail` `action: remedy`, then `remedy`, and it returned 30 s later. `orca-contract:doctored` was killed four times with `orca terminal close`, each 90 s after its `started` or `continued` line, by a script reading the journal; its three doctors (`-6`, `-7`, `-8`) each gave up. The log's script lines ended `agent() returns null, after 3 doctor rounds without a remedy`, then `contract holds`, and `summary.json` held `ok: true` and the object above exactly. The fold gave `contract:doctor` `round: 1` and `rounds` `[[1, 4, remedy]]`, and `orca-contract:doctored` `round: 3`, `doctors` `[6, 7, 8]` and three `gaveUp` rounds, each doctor with its `patient`. `r` opened the reclaim dialog, Esc said `nothing reclaimed` and the registry held no `reclaimed` line; `q` ended the runner, whose pid was then dead.
- **Resumed** from a new terminal: the three that returned were replayed, and the resume's `earlier` lines carried both patients' `round` and `rounds`, which the resumed fold read back unchanged. `orca-contract:doctored` ran live, was killed four times as above, and its three doctors (`-13`, `-14`, `-15`) each gave up before it returned null. `summary.json` held the same object, and the last script line was `contract holds`. The dialog reclaimed nothing on Esc, and `q` ended the runner.

Earlier pass: 2026-09-26 (#73), Orca 1.4.212 and Claude Code on macOS, with the runner as of #73's mailbox reading and remedy (402892f) and the script as of d4e9763, which adds `contract:doctor`, driven by an agent with the steps above, `orca terminal` verbs and the state dir's files only, launched with the preload. **The Orca-only contract holds, fresh and resumed** (Run `run_89086d0c798f`); the first resume failed on a kill the agent missed, and was run again (below).

- **Fresh, `contract:doctor`:** it started in the main checkout and, in every one of its four sessions, answered each prompt, nudge and continuation prompt with a hold like `Still no PROCEED. Not submitting.` and went idle. Each session was nudged twice, then continued in its own tab; the fourth death logged `the cap of 3` and `doctor round 1 of 3`, 26 minutes after its start. Its doctor started in `run_89086d0c798f-4`, read the patient's transcript, and sent a `handoff` whose note began `PROCEED.`, then `worker_done --outcome succeeded`. The runner journaled the handoff as `mail` (`kind: handoff`, `action: remedy`, with its `doctor`, `patient`, `round` and `body`), then `remedy` (`how: continue`, `reopened: false`, the patient's own dispatch and tab), and logged `doctor round 1 handed off a note; continuing session … with it in terminal …`. The patient submitted 32 s later, and the doctor's `worker_done` was journaled as `mail` with `action: ended`, then `settled` `succeeded`. The log said `contract:doctor returned {"value":{"word":"hello","count":3}}`. The runner's first mailbox check also drained the two `worker_done` messages `orca-contract:returned` and `contract:dirty-retry` had sent on submitting; each was journaled `action: none`.
- **Fresh, the rest:** `contract:dirty-retry`'s held create timed out at 600 s and the runner started it in the `-2` worktree Orca had made, as before. `orca-contract:doctored` was killed four times with `orca terminal close`, each 10 s after its transcript showed that session's `Bash` call of the wait; its three doctors (`-6`, `-7`, `-8`) each gave up over mail (`mail` with `action: gaveUp`, then `gaveUp` with `it gave up: <its body>`), and the log ended `agent() returns null, after 3 doctor rounds without a remedy`, then `contract holds`. `summary.json` held `ok: true` and the object above exactly. `r` opened the reclaim dialog with Reclaim All offered (`every agent of the run`) on the ended run while its runner waited on the view, which confirms that fix on Orca; Esc said `nothing reclaimed`, the registry held no `reclaimed` line, and `q` ended the runner. The view drew each doctor as a `└ recover` row under its patient.
- **The first resume, in the same state dir, failed:** it replayed `orca-contract:returned`, `contract:dirty-retry` and `contract:doctor`, but the agent's kill script took only journal lines written after its own start, and the runner had started `orca-contract:doctored` 20 s before it. So nobody killed it, its wait ran to its end, and it returned `{done: true}`: `FAIL doctored: expected null, got {"done":true}`. That result was journaled, so a further resume of that dir would only replay it.
- **Resumed again**, from a copy of the state dir whose journal was cut back to the 20 lines that first resume had written before it started `orca-contract:doctored`, as if its runner had died there: the carried-forward `run` line, the fresh run's 8 `mail` lines, the 8 `earlier` lines and the 3 replayed results. From a new terminal it took the Run over (a second `run` line, the same Run id and the new terminal), replayed the three, and journaled no `remedy`: the fresh run's handoff, carried forward as `mail`, was not applied again. `orca-contract:doctored` ran live, was killed four times as above, and its three doctors (`-16`, `-17`, `-18`) each gave up before it returned null. `summary.json` held the same object, and the last script line was `contract holds`. Esc on the dialog reclaimed nothing, the view drew all 7 doctors under their patients, and `q` ended it.
- **Seen, not changed:** every prompt again reached its Claude TUI only with the runner's nudge. On the first resume that nudge came 12 minutes after `orca-contract:doctored`'s start.
- Afterwards every worker and runner tab was closed with `orca terminal close`, and the eight child worktrees removed with `orca worktree rm --force`.

Earlier pass: 2026-09-26 (#72), Orca 1.4.212 on macOS, with the runner as of 2c2d73d, which has the doctor's `patient` fix and the operator-close fix below, driven by an agent with the steps above alone. **The Orca-only contract holds, fresh and resumed** (Run `run_4b6a81fa7208`).

That pass ran #72 as cut from #68, before it was rebased onto #69, #71 and #70: its script had no `contract:dirty-retry` case, and its runner no baseline or reclaim dialog. The rebased runner, with both `contract:dirty-retry` and `orca-contract:doctored` in one script, has passed the offline suite only, not yet this contract on Orca.

- **Fresh:** `orca-contract:returned` was nudged once and submitted. `orca-contract:doctored` was killed four times with `orca terminal close`, each time about 10 seconds after its transcript showed the wait's `Bash` call, and each continued session only once its `You were interrupted` prompt had started the wait anew. The fourth death logged `the cap of 3` and `doctor round 1 of 3`. The three doctors started in `run_4b6a81fa7208-3`, `-4` and `-5` and each gave up (`its worker settled failed`, `doctor round <k> of 3 ended without a remedy`). The journal has a `doctor`, `settled` (`outcome: failed`) and `gaveUp` line per round, then the patient's `failed`. The log ended `agent() returns null, after 3 doctor rounds without a remedy`, `orca-contract:doctored returned {"value":null}` and `contract holds`. `summary.json` held `ok: true` and the object above exactly. The run view drew the three doctors as `└ recover` rows under `orca-contract:doctored`. `n` logged `chosen: none`, `q` closed the view, and the runner's pid was then dead.
- **Resumed** from a new terminal: a second `run` line gave the same Run id and the new terminal, `orca-contract:returned` was replayed, and `orca-contract:doctored` ran live again. It was killed four times as above, and its three doctors (`-8`, `-9`, `-10`) each gave up before it returned null. `summary.json` held the same object, and the last script line was `contract holds`. The view drew both runs' doctors under their own patient row, which confirms the `patient` fix on an unbroken journal.
- **The fix it found:** the first fresh attempt on 594c88a (Run `run_41ae0c9c11c2`) returned the right object, but with no doctor. Its second kill logged `settled without submitting a result (outcome failed); agent() returns null`, 50 ms after the close. Orca 1.4.212 fails a closed tab's dispatch at once (`terminationReason: operator_close`), and `worker-show` can say so before it reports the terminal orphaned. A poll that lands in that gap reads the kill as a worker that settled. `orca-cli.mjs` now treats an `operator_close` dispatch as gone as well. The script's own check passed that attempt anyway, since a null reached it: the case's row asks for the doctor lines before the return, so read the log as well as `summary.json`.
- **Seen, not changed:**
  - Every prompt, the continuation prompts and the doctors' included, reached its Claude TUI only with the runner's nudge.
  - That nudge came about two minutes after the start on the fresh run, but 8 to 17 minutes after it on the resume. Orca's `terminal wait --for tui-idle` flipped between idle and timeout on an idle TUI holding an unsubmitted prompt. Each flip reads as a move, so the worker never looked still for long. The resume took 1h20m.
  - The run view marks a doctor that gave up `✓ done`.
- Afterwards every worker and runner tab was closed with `orca terminal close`, and the six doctor worktrees removed with `orca worktree rm`.

The pass before it, the same day on 0fe4ed4 (Run `run_e405a23d3ab4`), held fresh and resumed twice:

- As on #68's pass, `<worktree>` was the main checkout and the script paths pointed into the agent's own git worktree.
- **Fresh:** `orca-contract:returned` went idle without submitting after about two minutes, was nudged once, and submitted. `orca-contract:doctored` was killed four times with `orca terminal close`, each continued session about 45 seconds after its `continued` line; the fourth death logged `the cap of 3` and `doctor round 1 of 3`. Each of the three doctors started in its own worktree (`run_e405a23d3ab4-3`, `-4`, `-5`), made by `worktree create --setup skip`, read the patient's prompt and gave up with `worker_done --outcome failed`: the journal has a `doctor`, `settled` (`outcome: failed`) and `gaveUp` line per round, and only then the patient's `failed`. The log's script lines ended `orca-contract:doctored returned {"value":null}`, then `contract holds`; `summary.json` held `ok: true` and the object above exactly. `orca terminal read --screen` on the runner's tab showed the three doctors as `└ recover` rows under `orca-contract:doctored`. `n` logged `chosen: none`, and `q` closed the view; the runner's pid was then dead.
- **Resumed** from a new terminal: `orca-contract:returned` was replayed; `orca-contract:doctored`, journaled failed, ran live again, was killed four times, got three doctors that each gave up, and returned null. `summary.json` held the same object, and the last script line was `contract holds`.
- **The fix it found:** on that resume the fresh run's doctors were drawn as rows of their own, since a resume carries the patient's doctor lines nowhere and its `earlier` lines named no patient. A doctor's `earlier` line now carries its `patient`. A second resume on the fix drew the first resume's doctors under their patient, and returned the same object with `contract holds`. The fresh run's doctors stayed rows of their own there: their link was lost in the journal the unfixed resume wrote.
- **Seen, not changed:** each doctor's prompt reached its Claude TUI only with the runner's nudge, about two minutes after its start (its transcript's first line is timed just after the nudge), as `orca-contract:returned`'s did on this pass and on #68's. It holds for every worker, not only doctors.
- Afterwards every worker and runner tab was closed, and the nine doctor worktrees removed.

Earlier pass: 2026-09-26 (#71), Orca 1.4.212 and Claude Code on macOS, with the runner as of #71 and #69's worktree-create work merged into it, driven by an agent with the steps above alone and `orca terminal` verbs only, launched with the preload from `orca terminal create`. **The Orca-only contract holds, fresh and resumed, and the reclaim dialog reclaims nothing on Esc in the run's real Orca tab** (Run `run_fc7b258ee87f`).
