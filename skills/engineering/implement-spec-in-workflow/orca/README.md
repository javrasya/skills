...
| `earlier` | a resume carries forward, before any call, every other agent of the Run an earlier runner made: one that settled, or whose worker never started but left a worktree. It is no call, and replays nothing; it keeps the agent named for reclaim and the run view | `n`, `title`, `run`, `dispatchId`, `harness`, `sessionId`, `terminal`, `worktree`, `origin`, `state` and `reason` (as the journal last had them), `continuations` if its session was continued, and a doctor's `patient`, which keeps it under its patient in the run view |
