...
The agents the fresh run kept, the failed ones the resume keeps, and the child worktrees `contract:isolated` and `contract:retry` leave all stay until reclaimed: reclaim them from the resumed run's view (`r`, then Reclaim All once its runner has ended), or reclaim the run with `r` on the standalone runs list, which names every agent of the Run, the fresh run's included; or close their tabs and remove the `<runId>-<n>` worktrees by hand (`orca worktree rm`).
...
| Reclaim All | every agent; greyed out, with its reason, and not selectable while the runner is live (or nobody can say) |
