...
| a dead agent gets its doctors (ADR-0014) | `orca-contract:doctored` runs a nine-minute wait, and you kill it every time it runs: once, then in each of its 3 continuations, four kills in all. Past its cap it is the patient of three doctor rounds, one after another: each a `[Contract] recover -> orca-contract:doctored` worker in its own `<runId>-<n>` worktree, created with setup skipped. Its prompt tells a doctor to give up, so you kill no doctor: each settles `failed`, and the log says `doctor round <k> of 3 ended without a remedy`. The run view shows each doctor as a `└ recover` row under its patient | `doctored: null`, only after the third round: the log's `agent() returns null, after 3 doctor rounds without a remedy` comes before `orca-contract:doctored returned` |

Every run, fresh or resumed, must return exactly this:

```json
{"returned":{"count":3,"word":"hello"},"dirtyRetry":{"count":3,"word":"hello"},"doctored":null,"failures":[]}
```
...
Last pass: 2026-09-26 (#72), Orca 1.4.212 on macOS, with the runner as of 2c2d73d, which has the doctor's `patient` fix and the operator-close fix below, driven by an agent with the steps above alone. **The Orca-only contract holds, fresh and resumed** (Run `run_4b6a81fa7208`).
