...
| skill | needs |
|---|---|
| [pull-spiderman](./skills/engineering/pull-spiderman/SKILL.md) | the [`gh`](https://cli.github.com) CLI, authenticated against the repo; when the PR is a layer of a stack, also a clone of the repo (the sweep reads other layers' refs), git **worktree** support (the fix is built off the tip, never in your working copy), and a `docs/agents/issue-tracker.md` in the target repo for the review ticket |
| [stack-sweep](./skills/engineering/stack-sweep/SKILL.md) | `gh`, authenticated against the repo, run from inside a clone so it can `fetch` and read other layers' trees |
...
| [implement-spec-in-workflow](./skills/engineering/implement-spec-in-workflow/SKILL.md) | `gh`; git **worktree** support; a **runner** for a script of `agent()` calls — a workflow primitive such as Claude Code's `Workflow` tool (the Workflow runner), or Orca with Node (the Orca runner, ADR-0011); and the **`gh-stack` extension** for native stack registration — gated at arm time, never auto-installed; the run degrades to a plain `--base` chain (operator's choice) where the repo's stacks API is unavailable |
| [orca-runs](./skills/engineering/orca-runs/SKILL.md) | Node, a session inside an Orca terminal, `implement-spec-in-workflow` installed beside it (the view is its code), and npm with the network on the view's first start, to install terminal-kit |
...
- **[pull-spiderman](./skills/engineering/pull-spiderman/SKILL.md)** — Triage and answer the review comments on a PR (left by any reviewer — agent or human). Double-challenges each comment adversarially, drafts a short reply, and gates every reply/resolve/fix behind per-comment human approval. When the PR is a **layer of a stack**, it sweeps the layers above before judging any comment, never edits the layer, and lands every agreed fix as **one PR on the tip** tracked by one ticket (ADR-0010).
...
- **[orca-runs](./skills/engineering/orca-runs/SKILL.md)** — Open the Orca run view standalone, in a new Orca tab: every `implement-spec-in-workflow` run on the Orca runner, finished, paused or dead, grouped by project, with its outcome, whether its runner lives, what it keeps and its age. Opens a run into its live tree, reclaims a run's agents under the reclaim rules, and resumes a run whose runner died.
...
- **[code-review-in-stack](./skills/engineering/code-review-in-stack/SKILL.md)** — Review one PR of a stack against its own ticket's acceptance criteria: one verdict per criterion with named evidence, a sweep of the later layers before any work is recommended, and a **disposition** per gap (fix here when the PR is not stacked, fix in a PR on the tip when it is, hand forward as a ticket, no work). Reports to a screen: met criteria collapse to one line, only gaps get a block, drafts are offered rather than dumped. Edits no code and files nothing without approval.
...
- **[stack-sweep](./skills/engineering/stack-sweep/SKILL.md)** — Pin one PR's place in its stack (parents, children, siblings, tip, registered Stack or plain chain), then say for each construct — a gap's behaviour or a review comment's ask — whether a later layer **addresses** it, leaves it **unchanged through the tip**, or **changes it without addressing it**. Read-only; the shared query behind `code-review-in-stack` and stack-mode `pull-spiderman`.
...
`pull-spiderman` orchestrates the others: it delegates via `handover`, hardens fixes via `handover-loop`, runs its adversarial passes with `challenge`, and writes PR replies in the `caveman` voice. `handover-loop` builds on `handover`, which (like the subagents it spawns) uses `handoff` to pass context. When the PR it is given is a layer of a stack, it also reaches for `stack-sweep` before judging any comment, and its fixes stop going into the PR: a published layer is immutable, so every agreed fix of one run becomes one PR on the tip, tracked by one ticket, and the reply says where the answer went (ADR-0010).
...
`code-review-in-stack` is what reads a stack back: where `code-review` judges a diff on standards and spec, this one judges a slice against the criteria of the ticket it claims, and it looks at the layers *above* the PR before recommending a fix — a gap already addressed at layer 15 is not work. It reaches for `code-review` when the standards axis is wanted too, reports the axes side by side, and hands anything needing a decision to a ticket rather than into the PR under review. It never writes code, which is the line between it and `pull-spiderman`: that one answers a reviewer and may fix; this one *is* the reviewer.

`stack-sweep` is the one query both of them share, and the only piece deliberately factored out: pin the stack, then for each construct return one of three outcomes with per-layer evidence. `code-review-in-stack` hands it a gap's behaviour; `pull-spiderman` hands it a comment's ask. Neither restates the query, because a second copy is how two skills come to mean different things by "addressed downstream".
